...
exports.NinebotClient = void 0;
exports.parseVehicleState = parseVehicleState;
...
class NinebotClient {
    ...
    async getVehicleSnapshot(vehicle) {
...
exports.NinebotClient = NinebotClient;
function parseVehicleState(statusPayload, travelPayload, batteryPayload) {
...
function firstArrayObject(object, keys) {
    for (const key of keys) {
...
function firstBoolean(objects, keys, trueValue) {
    for (const object of objects) {
        for (const key of keys) {
            const value = object[key];
...
function normalizeEnergy(value, distanceKm) {
    ...
    if (distanceKm && distanceKm > 0) {
        ...
        if (plausible.length) {
            return plausible.reduce((best, candidate) => Math.abs(Math.log(candidate / distanceKm) - Math.log(38)) < Math.abs(Math.log(best / distanceKm) - Math.log(38)) ? candidate : best);
        }
    }
...
//# sourceMappingURL=ninebot-client.js.map
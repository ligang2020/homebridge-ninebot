...
export class NinebotPlatform {
  ...
  private configureServices(accessory: PlatformAccessory, vehicle: NinebotVehicle): void {
    ...
    battery.getCharacteristic(this.Characteristic.StatusLowBattery).onGet(() => this.readState(vehicle.sn).then((state) =>
    ...
    temperature.getCharacteristic(this.Characteristic.CurrentTemperature).onGet(() =>
      this.readState(vehicle.sn).then((state) => state.batteryTemperature ?? 0),
    );
    ...
    online.getCharacteristic(this.Characteristic.OccupancyDetected).onGet(() =>
      this.readState(vehicle.sn).then((state) => state.isPoweredOn ? this.Characteristic.OccupancyDetected.OCCUPANCY_DETECTED : this.Characteristic.OccupancyDetected.OCCUPANCY_NOT_DETECTED),
    );
  ...
  private configureMetricGetters(service: any, sn: string): void {
    ...
    for (const [characteristicName, stateKey] of Object.entries(mappings) as [MetricKey, MetricKey][]) {
      service.getCharacteristic(characteristics[characteristicName]).onGet(() =>
        this.readState(sn).then((state) => state[stateKey] ?? 0),
      );
    }
  }
  ...
  private applyState(accessory: PlatformAccessory, snapshot: NinebotVehicleSnapshot): void {
    ...
    if (lock) {
      ...
      lock.updateCharacteristic(this.Characteristic.LockTargetState,
        state.isLocked === true ? this.Characteristic.LockTargetState.SECURED : this.Characteristic.LockTargetState.UNSECURED,
      );
    }
  ...
  private readLockCurrentState(sn: string): Promise<number> {
  ...
  private readLockTargetState(sn: string): Promise<number> {
    return this.readState(sn).then((state) => state.isLocked === true

...
| 电池温度 | BMS / 电池温度 |
| 车辆电源 | 远程启动、关闭发动机 |
| 车辆锁状态 | 只读状态传感器：已锁车对应“关闭”，未锁车对应“打开”；不会显示为可解锁的按钮 |
...
服务名称会在每次插件启动和车辆刷新时同步为 **车辆电源**、**寻车响铃**、**打开坐桶**、**电池**、**充电器** 等正确名称。对于曾经被 Apple Home 缓存为车辆名称的旧磁贴，v1.0.8 会仅一次性替换受影响的服务身份并保留 Bridge 配对和车辆配件 UUID；重启 Homebridge 后，Apple Home 会以正确的服务名称重新建立这些磁贴。

“车辆状态与骑行数据”使用 HomeKit 自定义特征，因此在 **Eve、Controller for HomeKit、Home+** 等高级客户端可完整查看电压、剩余充电时间和坐标；Apple 家庭 App 对自定义数值特征的显示有限，但会正常显示标准的电池、充电器、温度和只读锁状态。车辆位置由 Proxy 返回的经纬度提供；家庭 App 没有原生的车辆地图服务。
...
- 锁车接口尚未包含在当前 Proxy API 中，因此插件将“车辆锁状态”作为**只读状态传感器**显示：已锁车为“关闭”，未锁车为“打开”；不会伪造“锁车/解锁成功”。
